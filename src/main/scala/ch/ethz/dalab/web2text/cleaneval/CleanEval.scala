...
object CleanEval {
  ...
  val web2textTestSet: Vector[Int] = Vector(309,87,534,705,362,192,247,538,311,29,494,202,210,636,183,238,628,64
                                   ...
                                   ,118,333,437,254).map(i => indices(i))

  val cleanevalTestSet: Vector[Int] = Vector(
    ...
    729,730,731,732,733,734,735,736).map(i => indices(i))
  val web2textTrainingSet: Vector[Int] = Vector(133,614,30,546,317,85,544,115,657,607,231,331,4,555,274,194,310,515
                                       ...
                                       ,316,483,645,298,179,75,730,124,78,188,112,193,516).map(i => indices(i))
  ...
  def web2textTrainingIterator: Iterator[Page] =
    web2textTrainingSet.toIterator.map {i => Page(i) }

  def web2textTestIterator: Iterator[Page] =
    web2textTestSet.toIterator.map {i => Page(i) }

  def cleanevalTestIterator: Iterator[Page] =
    cleanevalTestSet.toIterator.map {i => Page(i) }
  ...
  def trainingDataset(extractor: FeatureExtractor): Vector[(PageFeatures,Vector[Int])] = {
      val it = web2textTrainingIterator
  ...
  def evaluateCleaner(alignedLocation: Int=>String): PerformanceStatistics = {
    val pairs = cleanevalTestIterator flatMap { p =>
