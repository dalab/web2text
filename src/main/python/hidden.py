...
tf.app.flags.DEFINE_string('train', "\\trained_model_web2text_split\\train\\output_7__0a0a5bddf78b2ef2de13c71fbf737764bbf97449_block_features.csv",
                           'File containing the training data (labels & features).')
tf.app.flags.DEFINE_string('test',  "\\trained_model_web2text_split\\test\\output_7__0a0a5bddf78b2ef2de13c71fbf737764bbf97449_block_features.csv",
                           'File containing the test data (labels & features).')
tf.app.flags.DEFINE_integer('num_epochs', 1,
                            'Number of passes over the training data.')
tf.app.flags.DEFINE_integer('num_hidden', NUM_HIDDEN,
...
def get_next_batch(offset,  file_sizes_list):
    batch = []
    file_start = 0
    file_start_offset = 0
    file_end = 0
    ...
    count_files = 0
    i = 0
    while not i>= TRAIN_SIZE:




    return batch, offset_latest_file
...
def extract_data(filename):
    ...
    i = 0 # From 0 to 140, one for each feature
    for line in file(filename):
        i+=1
        if i not in [30,54,98,120]: # Skip these ones because they are not features
            fvec = line.split(",")
            j = 0
            for tex in row:
                fvecs[]
...
def main(argv=None):
    ...
    # Get the file list
    train_file_sizes_list = generate_queue(TRAIN_FILE_PATH)
    test_file_list = generate_queue(TEST_FILE_PATH)

    offset = 0
    offset_latest_file = 0
