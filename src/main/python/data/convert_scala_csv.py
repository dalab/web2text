...
def cli(block_csv, npy_output):

  #click.echo("Converting '%s' and '%s' to '%s'" % (block_csv, edge_csv, npy_output))
  click.echo("Converting '%s' to '%s'" % (block_csv, npy_output))

  data = np.genfromtxt(block_csv, delimiter=',') # returns an ndarray
  # edge_data = np.genfromtxt(edge_csv, delimiter=',')
  click.echo("- CSV file loaded")

  def docs(data):
    i = 0
    n = data.shape[0] # Number of rows (=140)
  ...
  click.echo("- Data converted into document format: list({data, labels})")
